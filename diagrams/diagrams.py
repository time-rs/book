...
year_repr = Sequence("repr:", Choice(0, "full", "century", "last_two"))
...
unix_timestamp_precision = Sequence(
    "precision:", Choice(0, "second", "millisecond", "microsecond", "nanosecond")
)

whitespace = Comment("whitespace")

# region: abbreviated diagram
abbreviated_v1 = NamedDiagram(
    "abbreviated-v1",
    Diagram(
        ZeroOrMore(
            Choice(
...
month = generate_diagram("month", copy(padding), copy(month_repr), copy(case_sensitive))
ordinal = generate_diagram("ordinal", copy(padding))
weekday = generate_diagram(
    "weekday", copy(weekday_repr), copy(weekday_one_indexed), copy(case_sensitive)
)
week_number = generate_diagram("week_number", copy(padding), copy(week_number_repr))
year = generate_diagram(
    "year",
    copy(padding),
    copy(year_repr),
    copy(year_range),
